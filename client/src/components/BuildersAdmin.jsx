...
const S = {
  ...
  addBtn: { background: 'linear-gradient(135deg, #A1824A, #8F723E)', color: '#fff', border: 'none', borderRadius: 8, padding: '10px 20px', fontWeight: 700, fontSize: 13, cursor: 'pointer' },
      ...
      {activeTool === 'fieldguide' && (
        <div style={{ display: 'flex', gap: 6 }}>
          {FIELDGUIDE_SECTIONS.map(s => (
            <button key={s.id} style={{ ...S.btn(fgSection === s.id ? '#9c7c3a' : undefined), fontSize: 10 }} onClick={() => setFgSection(s.id)}>
                ...
                <tr key={item.id}>
                  ...
                  <td style={S.td}>
                    <div style={S.actions}>
                      ...
                      <button style={S.btn('#9c7c3a')} onClick={() => setEditingItem({ ...item })}>✏️</button>
