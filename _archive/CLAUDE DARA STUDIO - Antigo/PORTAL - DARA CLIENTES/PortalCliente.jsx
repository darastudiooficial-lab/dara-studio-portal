...
/* ─────────────────────────────────────────────
   ...
   Dashboard data queries:
     - projects: supabase.from('projects').select('*').eq('client_id', user.id)
     - invoices:  supabase.from('invoices').select('*').eq('client_id', user.id)
     - files:     supabase.from('project_files').select('*').eq('client_id', user.id)
     - messages:  supabase.from('messages').select('*').eq('client_id', user.id).eq('read', false)
...
const MOCK_PROJECTS = [
  {
    ...
    pendingAction: "Aguardando informações do cliente",
  },
  {
    id: 2,
    code: "DARA-0008",
...
:root{
  ...
  --sidebar:#1a1814;--sidebar2:#222018;--sidebar3:#2e2b22;
  --text:#1a1814;--muted:#6b6760;--dim:#a09b94;--border:#e2ddd6;
  --green:#2d8653;--green-bg:rgba(45,134,83,0.1);
  --amber:#c07c0a;--amber-bg:rgba(192,124,10,0.1);
  --red:#b83232;--red-bg:rgba(184,50,50,0.09);
  --blue:#2566a8;--blue-bg:rgba(37,102,168,0.09);
...
const statusLabel = { on_track: "On Track", attention: "Atenção", completed: "Concluído" };
const statusColor = {
...
function LoginScreen({ onLogin }) {
  ...
  // MOCK CREDENTIALS — substituir por supabase.auth.signInWithPassword() no futuro
  const VALID_EMAIL = "joao@darastudio.com";
        ...
        <div style={{ marginBottom: error ? 8 : 20 }}>
          ...
          <div style={{ position:"relative" }}>
            <input type={showPw?"text":"password"} placeholder="••••••••" value={password} onChange={e=>setPassword(e.target.value)}
              ...
              onBlur={e=>e.target.style.borderColor="var(--border)"}
              onKeyDown={e=>e.key==="Enter"&&handleSubmit()}/>
            <button onClick={()=>setShowPw(p=>!p)} style={{ position:"absolute", right:10, top:"50%", transform:"translateY(-50%)", background:"none", border:"none", color:"var(--dim)", padding:4 }}>
    ...
    <div style={{ display:"flex", minHeight:"100vh", background:"var(--bg)" }}>
      ...
      <aside style={{ width: sidebarOpen ? 220 : 56, background:"var(--sidebar)", flexShrink:0,
        ...
        <div style={{ padding: sidebarOpen ? "20px 16px 16px" : "20px 0 16px", display:"flex",
          ...
          <button onClick={()=>setSidebarOpen(o=>!o)} style={{ background:"none", border:"none",
      ...
      <main style={{ flex:1, overflow:"auto", minWidth:0 }}>
        ...
        <div style={{ background:"var(--bg2)", borderBottom:"1px solid var(--border)", padding:"0 28px",
          ...
          <div style={{ display:"flex", alignItems:"center", gap:12 }}>
            <div style={{ position:"relative" }}>
              <button onClick={()=>{ setShowNotif(o=>!o); }} style={{ background:"none", border:"none", color:"var(--muted)", position:"relative", padding:4, cursor:"pointer" }}>
                <Bell size={16}/>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:8 }}>
              <div style={{ width:30,height:30, borderRadius:"50%", background:"var(--gold-d)",
                ...
                <span style={{ fontSize:11, fontWeight:700, color:"var(--gold)" }}>{MOCK_USER.initials}</span>
              </div>
              {sidebarOpen && <span style={{ fontSize:12, fontWeight:500, color:"var(--text)" }}>{MOCK_USER.name.split(" ")[0]}</span>}
            </div>
          </div>
        </div>
        ...
        <div style={{ padding:"28px 28px 60px" }}>
          {page === "dashboard"      && <PageDashboard onNavigate={(p)=>{ if(p==="new_project") setNewProjectModal(true); else setPage(p); }} onOpenProject={openProject} pendingInvoices={pendingInvoices} totalPaid={totalPaid} unreadMsg={unreadMsg}/>}
          {page === "projects"       && <PageProjects onOpenProject={openProject} />}
        ...
        <div style={{ position:"fixed", inset:0, zIndex:300, overflowY:"auto" }}>
          <EstimateForm
            ...
            onSubmit={(formData)=>{
              const newProject = {
                id: Date.now(),
                code: `DARA-${String(MOCK_PROJECTS.length+1).padStart(4,"0")}`,
                address: formData.city || "Novo Projeto",
                city: `${formData.city||""}, ${formData.state||""}`,
                service: (formData.service||"estimate").replace(/_/g," "),
                stage: "Levantamento",
                status: "on_track",
                budget: 0, paid: 0, progress: 0,
...
//  PAGE: DASHBOARD
// ════════════════════════════════════════════
function PageDashboard({ onNavigate, onOpenProject, pendingInvoices, totalPaid, unreadMsg }) {
  const activeProjects = MOCK_PROJECTS.filter(p=>p.status!=="completed").length;

  const metricCards = [
    { label:"Active Projects", value: activeProjects, sub: `${MOCK_PROJECTS.filter(p=>p.status==="completed").length} completed`, icon:FolderKanban, color:"var(--gold)", onClick:()=>onNavigate("projects") },
    { label:"Pending Invoices", value: pendingInvoices, sub:`$${MOCK_INVOICES.filter(i=>i.status==="pending").reduce((s,i)=>s+i.amount,0).toLocaleString()} outstanding`, icon:Receipt, color:"var(--amber)", onClick:()=>onNavigate("invoices") },
...
//  PAGE: MY PROJECTS
// ════════════════════════════════════════════
function PageProjects({ onOpenProject }) {
  ...
  const doEdit = () => {
    ...
    const p = MOCK_PROJECTS.find(p=>p.id===editModal);
    if(p) p.address = editName.trim();
    setProjects([...MOCK_PROJECTS]);
    setEditModal(null);
  };
...
function PageProjectDetail({ project, onBack }) {
  ...
  const [previewFile, setPreviewFile] = useState(null); // for drawings/files preview modal
  ...
  const sendMsg = () => {
    ...
    setMessages(m=>[...m,{id:Date.now(),from:"client",name:"João",text:chatMsg.trim(),time:"agora"}]);
    setChatMsg("");
  };
  const postUpdate = () => {
    if (!siteText.trim()) return;
    setSiteUpdates(u=>[{id:Date.now(),date:new Date().toLocaleDateString("pt-BR"),author:"João (cliente)",text:siteText.trim(),photos:sitePhotos.length},...u]);
    setSiteText(""); setSitePhotos([]);
  };
  const submitTicket = () => {
    if (!ticketData.desc.trim()) return;
    setTickets(t=>[{id:`TKT-${String(t.length+1).padStart(3,"0")}`,type:ticketData.type,ref:ticketData.ref,desc:ticketData.desc,status:"Aberto",date:new Date().toLocaleDateString("pt-BR")},...t]);
    ...
    <div className="fade-up">
      ...
      {activeTab==="overview" && (
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
          {/* Client info — editable by client */}
          <div style={{ background:"var(--bg2)", border:"1px solid var(--border)", borderRadius:10, padding:"18px 20px" }}>
            ...
            {[["Full Name","Jackson Da Silva"],["Email","jackgeneraloffice@gmail.com"],["Phone","+1 (617) 775-0179"]].map(([label,val])=>(
              <div key={label} style={{ marginBottom:10 }}>
                <label style={{ fontSize:10, fontWeight:600, color:"var(--dim)", letterSpacing:"0.06em",
                  textTransform:"uppercase", display:"block", marginBottom:4 }}>{label}</label>
                <input defaultValue={val} style={{ width:"100%", border:"1.5px solid var(--border)",
                  borderRadius:7, padding:"8px 11px", fontSize:13, color:"var(--text)",
                  background:"var(--bg)", fontFamily:"var(--sans)", outline:"none" }}
                  onFocus={e=>e.target.style.borderColor="var(--gold)"}
                  onBlur={e=>e.target.style.borderColor="var(--border)"}/>
              </div>
            ))}
            <button style={{ background:"var(--gold)", border:"none", borderRadius:7, padding:"8px 18px",
              fontSize:12, fontWeight:700, color:"#fff", cursor:"pointer", marginTop:4 }}>
              Salvar Alterações
            </button>
          </div>
          ...
          <div>
            <div style={{ background:"var(--bg2)", border:"1px solid var(--border)", borderRadius:10,
              ...
              <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:14 }}>
                ...
                <span style={{ fontSize:9, fontWeight:600, letterSpacing:"0.08em", color:"var(--dim)",
                  background:"var(--bg3)", borderRadius:3, padding:"1px 6px", marginLeft:4 }}>SOMENTE LEITURA</span>
              </div>
              {[
                ["Endereço", `${project.address} — ${project.city}`],
                ["Área", "2,000 sqft"],
                ["Níveis", "Ground + 2nd Floor"],
                ["Início", "15/01/2026"],
                ["Arquiteto", "Daniela Araújo"],
                ["Tipo de Serviço", project.service],
              ].map(([k,v])=>(
                <div key={k} style={{ display:"flex", justifyContent:"space-between",
                  padding:"7px 0", borderBottom:"1px solid rgba(0,0,0,0.04)" }}>
                  <span style={{ fontSize:11, color:"var(--muted)" }}>{k}</span>
                  <span style={{ fontSize:11, fontWeight:500, color:"var(--text)", textAlign:"right",
                    maxWidth:"55%" }}>{v}</span>
                </div>
              ))}
            </div>
      ...
      {activeTab==="updates" && (
        <div>
          ...
          {siteUpdates.map(u=>(
            <div key={u.id} style={{ background:"var(--bg2)", border:"1px solid var(--border)",
              borderRadius:10, padding:"14px 18px", marginBottom:10 }}>
              <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
                <span style={{ fontSize:11, color:"var(--gold)", fontWeight:600 }}>{u.author}</span>
                <span style={{ fontSize:10, color:"var(--dim)" }}>{u.date}</span>
              </div>
                        ...
                        {inv.status==="pending" && (
                          <div style={{ marginTop:12 }}>
                            <label style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:6,
                              background: payProof[ph.id] ? "var(--green)" : "var(--gold)",
                              border:"none", borderRadius:8, padding:"9px",
                              fontSize:12, fontWeight:700, color:"#fff", cursor:"pointer", width:"100%" }}>
                              📎 {payProof[ph.id] ? `✓ ${payProof[ph.id]}` : "Anexar Comprovante"}
                              <input type="file" accept="image/*,.pdf" style={{ display:"none" }}
                                onChange={e=>{ const f=e.target.files?.[0]; if(f) setPayProof(p=>({...p,[ph.id]:f.name})); }}/>
                            </label>
                          </div>
                        )}
                      </div>
              ...
              <div key={m.id} style={{ display:"flex",
                ...
                <div style={{ maxWidth:"68%" }}>
                  <div style={{ background:m.from==="client"?"rgba(184,155,106,0.12)":"var(--bg3)",
                    border:`1px solid ${m.from==="client"?"rgba(184,155,106,0.2)":"var(--border)"}`,
                  ...
                  <p style={{ fontSize:10, color:"var(--dim)", marginTop:2,
                    textAlign:m.from==="client"?"right":"left" }}>{m.name} · {m.time}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {activeTab==="tickets" && (
        <div>
          ...
          {tickets.length===0 ? (
            ...
            <div key={t.id} style={{ background:"var(--bg2)", border:"1px solid var(--border)",
              borderRadius:10, padding:"14px 18px", marginBottom:10 }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                <div>
                  <div style={{ display:"flex", gap:8, alignItems:"center", marginBottom:5 }}>
                    <span style={{ fontFamily:"monospace", fontSize:11, color:"var(--gold)", fontWeight:700 }}>{t.id}</span>
                    <span style={{ fontSize:10, background:"var(--amber-bg)", color:"var(--amber)",
                      borderRadius:3, padding:"1px 6px", fontWeight:600 }}>{t.status}</span>
...
function PageInvoices() {
  ...
  // ── mock data ──
  const totalPaid      = MOCK_INVOICES.filter(i=>i.status==="paid").reduce((s,i)=>s+i.amount,0);
  const totalOutstanding = MOCK_INVOICES.filter(i=>i.status==="pending").reduce((s,i)=>s+i.amount,0);
...
function PageProfile() {
  ...
  const [emailUser, setEmailUser]   = useState("jackgeneraloffice");
  const [emailDomain, setEmailDomain] = useState("@gmail.com");
  const [showDomains, setShowDomains] = useState(false);
  const [phone, setPhone]       = useState("+1 (617) 775-0179");
  const [igHandle, setIgHandle] = useState("jackgeneralservices");
            ...
            <div style={{ position:"relative", flexShrink:0 }}>
              <div style={{ width:64, height:64, borderRadius:"50%", background:"var(--gold-d)",
                ...
                {avatarUrl
                  ...
                  : <span style={{ fontFamily:"var(--serif)", fontSize:22, fontWeight:700, color:"var(--gold)" }}>{MOCK_USER.initials}</span>
                }
              </div>
              {/* Camera overlay */}
    ...
    <div className="fade-up" style={{ maxWidth:560 }}>
      ...
      <div style={{ background:"var(--bg2)", border:"1px solid var(--border)", borderRadius:12, padding:"24px" }}>
        ...
        <div style={{ display:"flex", justifyContent:"flex-end", gap:10, marginTop:20 }}>
          ...
          <button onClick={()=>setSavedCo(true)}
            style={{ background:"var(--gold)", border:"none", borderRadius:8, padding:"9px 22px",
              fontSize:13, fontWeight:700, color:"#fff", cursor:"pointer" }}>
            Salvar Alterações
          </button>
...
function eCalcConf(data) {
  const pts = [data.country, data.city, data.state, data.width, data.length,
    (data.levels||[]).length > 0, data.service, data.projectType,
...
// ── STEP PROGRESS ──
const E_STEPS = ["Location","Details","Scope","Rooms","Files","Rush","Review"];
...
function EStep1({ data, set }) {
  ...
  const stateList = data.country === "BR" ? BR_STATES : US_STATES;

  return (
    <div>
      <h2 style={{ fontFamily:E.serif, fontSize:17, fontWeight:700, marginBottom:5 }}>Project Location</h2>
      <p style={{ fontSize:12, color:E.muted, marginBottom:20 }}>Select your region and provide the project address.</p>

      {/* Country toggle */}
      <div style={{ display:"flex", gap:10, marginBottom:20 }}>
        {[["US","United States","USD · sqft"],["BR","Brazil","BRL · m²"]].map(([code,name,sub])=>(
          <button key={code} onClick={()=>{ set("country",code); set("state",""); }} style={{
      ...
      {/* Address fields — all required, in requested order */}
      <div style={{ display:"grid", gap:13 }}>
        {/* Street Address */}
        <EInput label="Street Address" required value={data.street||""} onChange={e=>set("street",e.target.value)} placeholder="123 Main St"/>

        {/* City */}
        <EInput label="City" required value={data.city||""} onChange={e=>set("city",e.target.value)} placeholder="e.g. Boston"/>

        {/* State — dropdown */}
        <div>
          <label style={{ fontSize:11, fontWeight:600, color:E.muted, letterSpacing:"0.06em",
            textTransform:"uppercase", display:"block", marginBottom:6 }}>
            State / Province <span style={{ color:E.red }}>*</span>
          </label>
          <div style={{ position:"relative" }}>
            <button onClick={()=>setStateOpen(o=>!o)} style={{
              width:"100%", background:E.bg3, border:`1.5px solid ${stateOpen ? E.gold : E.border}`,
              borderRadius:8, padding:"9px 12px", fontSize:13, color: data.state ? E.text : E.dim,
              fontFamily:E.sans, textAlign:"left", cursor:"pointer",
              display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              {data.state || "Select state / province..."}
              <span style={{ fontSize:10, color:E.dim }}>▾</span>
            </button>
            {stateOpen && (
              <>
                <div onClick={()=>setStateOpen(false)} style={{ position:"fixed", inset:0, zIndex:49 }}/>
                <div style={{ position:"absolute", top:"100%", left:0, right:0, zIndex:50, marginTop:4,
                  background:"#fff", border:`1px solid ${E.border}`, borderRadius:8,
                  ...
                  {stateList.map(s=>(
                    <div key={s} onClick={()=>{ set("state",s); setStateOpen(false); }}
                      ...
                      {data.state===s && <span style={{ marginRight:7, fontSize:10 }}>✓</span>}
                      {s}
                    </div>
                  ))}
                </div>
        ...
        {/* ZIP */}
        <EInput label="ZIP / Postal Code" required value={data.zip||""} onChange={e=>set("zip",e.target.value)} placeholder="e.g. 02134"/>
      </div>
    </div>
  );
}
...
function EStep2({ data, set }) {
  const sqft = data.width && data.length ? data.width * data.length : null;
  const levels = ["Ground Floor / Main Level","2nd Floor","Basement","Attic"];
  const toggleLv = (lv) => {
  ...
  return (
    <div>
      <h2 style={{ fontFamily:E.serif, fontSize:17, fontWeight:700, marginBottom:5 }}>Project Details</h2>
      <p style={{ fontSize:12, color:E.muted, marginBottom:20 }}>Enter your project dimensions and select the levels included.</p>
      <div style={{ marginBottom:16 }}>
        <label style={{ fontSize:11, fontWeight:600, color:E.muted, letterSpacing:"0.06em",
          textTransform:"uppercase", display:"block", marginBottom:6 }}>Base Dimensions *</label>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
          <EInput value={data.width||""} onChange={e=>set("width",Number(e.target.value)||"")}
            placeholder="Width (ft)" type="number" min="1"/>
          <EInput value={data.length||""} onChange={e=>set("length",Number(e.target.value)||"")}
            placeholder="Length (ft)" type="number" min="1"/>
        </div>
        {sqft && (
          <div style={{ marginTop:8, padding:"8px 12px", background:E.goldD,
            border:`1px solid ${E.goldBorder}`, borderRadius:6, display:"flex", justifyContent:"space-between" }}>
            <span style={{ fontSize:12, color:E.muted }}>Total Base Area</span>
            <span style={{ fontSize:12, fontWeight:700, color:E.gold }}>{sqft.toLocaleString()} sqft</span>
          </div>
        )}
      </div>
      <div style={{ marginBottom:16 }}>
        <EInput label="Lot Size (optional)" value={data.lotSize||""} onChange={e=>set("lotSize",e.target.value)}
          placeholder="e.g. 0.25 acres"/>
      </div>
      <div>
        <label style={{ fontSize:11, fontWeight:600, color:E.muted, letterSpacing:"0.06em",
          textTransform:"uppercase", display:"block", marginBottom:8 }}>Levels *</label>
        <div style={{ display:"flex", flexDirection:"column", gap:7 }}>
          {levels.map(lv=>{
            const active = (data.levels||[]).includes(lv);
            const isDefault = lv === "Ground Floor / Main Level";
            return (
              <button key={lv} onClick={()=>!isDefault && toggleLv(lv)} style={{
                display:"flex", alignItems:"center", justifyContent:"space-between",
                background: active ? E.goldD : E.bg3,
                border:`1.5px solid ${active ? E.gold : E.border}`,
                borderRadius:8, padding:"10px 14px", cursor: isDefault?"default":"pointer" }}>
                <span style={{ fontSize:13, color: active ? E.text : E.muted }}>{lv}</span>
                <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                  {isDefault && <span style={{ fontSize:10, color:E.dim, background:E.bg2, borderRadius:3, padding:"1px 6px" }}>DEFAULT</span>}
                  {lv==="Basement"&&!active && <span style={{ fontSize:10, color:E.amber }}>+UPCHARGE</span>}
                  <div style={{ width:16, height:16, borderRadius:3,
                    background: active ? E.gold : "transparent",
                    border:`1.5px solid ${active ? E.gold : E.border}`,
                    display:"flex", alignItems:"center", justifyContent:"center" }}>
                    {active && <CheckCircle2 size={9} color="#fff" strokeWidth={3}/>}
                  </div>
                </div>
              </button>
            );
...
function EStep5({ data, set }) {
  ...
  const removeFile = (name) => set("fileList", files.filter(f => f.name !== name));
    ...
    <div>
      ...
      <div style={{ background:E.bg3, border:`1px solid ${E.border}`, borderRadius:8,
        ...
        <p style={{ fontSize:10, color:E.dim, marginTop:8 }}>
          PDF, JPG, PNG, DWG, MP4, MOV — up to 10 files
        </p>
      </div>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:7 }}>
          {files.map(f=>(
            <div key={f.name} style={{ display:"flex", alignItems:"center", gap:11,
              padding:"10px 13px", background:E.greenBg,
              ...
              <button onClick={()=>removeFile(f.name)} style={{ background:"none",
                border:"none", color:E.dim, cursor:"pointer", padding:2, flexShrink:0 }}>
                <X size={13}/>
              </button>
            </div>
          ))}
          <p style={{ fontSize:11, color:E.green, marginTop:4, display:"flex", alignItems:"center", gap:5 }}>
            <CheckCircle2 size={12} color={E.green}/> {files.length} arquivo(s) selecionado(s)
          </p>
        </div>
      )}
    </div>
...
function EStep7({ data, onSubmit }) {
  ...
  return (
    <div>
      <h2 style={{ fontFamily:E.serif, fontSize:17, fontWeight:700, marginBottom:5 }}>Review & Submit</h2>
      <p style={{ fontSize:12, color:E.muted, marginBottom:18 }}>Confirm your project details and estimated fee below.</p>
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:11, marginBottom:14 }}>
        {[
          {title:"Client & Location", rows:[
            ["City / State", `${data.city||"–"}, ${data.state||"–"}`],
            ["Country", data.country==="BR"?"Brazil":"United States"],
          ]},
          {title:"Project Dimensions", rows:[
            ["Width (ft)", data.width||"–"],
            ["Length (ft)", data.length||"–"],
            ["Base Area", sqft ? `${sqft} sqft` : "–"],
          ]},
          {title:"Scope & Services", rows:[
            ["Project Type", (data.projectType||"–").replace(/_/g," ")],
            ["Engagement", SVC_LABEL[data.service]||"–"],
            ["Files Uploaded", `${uploadedCount} / ${E_UPLOAD_DOCS.length}`],
          ]},
        ].map(card=>(
          <div key={card.title} style={{ background:E.bg3, border:`1px solid ${E.border}`,
            borderRadius:9, padding:"13px 15px" }}>
            <p style={{ fontSize:10, fontWeight:700, color:E.gold, letterSpacing:"0.09em",
              textTransform:"uppercase", marginBottom:10 }}>{card.title}</p>
            {card.rows.map(([k,v])=>(
              <div key={k} style={{ display:"flex", justifyContent:"space-between",
                padding:"5px 0", borderBottom:`1px solid ${E.border2}` }}>
                <span style={{ fontSize:11, color:E.muted }}>{k}</span>
        ...
        {price && (
          <div style={{ background:E.goldD, border:`1px solid ${E.goldBorder}`,
            borderRadius:9, padding:"13px 15px" }}>
            <p style={{ fontSize:10, fontWeight:700, color:E.gold, letterSpacing:"0.09em",
              textTransform:"uppercase", marginBottom:10 }}>Estimated Design Fee</p>
            <p style={{ fontFamily:E.serif, fontSize:22, fontWeight:700, color:E.text, marginBottom:8 }}>
              ${price.low.toLocaleString()} – ${price.high.toLocaleString()}
            </p>
            {[
              [SVC_LABEL[data.service]||"Service", `$${price.base}`],
      ...
      <div style={{ padding:"12px 13px", background:E.redBg,
      ...
      <ECheckbox checked={agreed} onChange={()=>setAgreed(a=>!a)}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginTop:20 }}>
        <button onClick={()=>agreed&&onSubmit()} disabled={!agreed} style={{
          background: agreed ? E.gold : E.bg3,
          border:`1px solid ${agreed ? E.gold : E.border}`,
          borderRadius:9, padding:"12px", fontSize:14, fontWeight:700,
          color: agreed ? "#fff" : E.dim, cursor: agreed?"pointer":"not-allowed",
          display:"flex", alignItems:"center", justifyContent:"center", gap:7 }}>
          <Zap size={15}/> Accept & Continue
        </button>
        <button style={{ background: savedDraft ? E.greenBg : "none",
          border:`1px solid ${savedDraft ? "rgba(45,134,83,0.3)" : E.border}`,
          borderRadius:9, padding:"12px", fontSize:13,
          color: savedDraft ? E.green : E.muted, cursor:"pointer",
          display:"flex", alignItems:"center", justifyContent:"center", gap:6,
          transition:"all 0.2s" }}
...
function EstimateForm({ onClose, onSubmit }) {
  ...
  const canNext = () => {
    if(step===0) return !!(data.country && data.street && data.city && data.state && data.zip);
  ...
  const steps = [EStep1,EStep2,EStep3,EStep4,EStep5,EStep6,EStep7];
        ...
        <div style={{ maxWidth:900, margin:"0 auto", padding:"28px 24px" }}>
          ...
          <div style={{ display:"flex", gap:22, alignItems:"flex-start" }}>
            <div style={{ flex:1, minWidth:0, background:E.bg2, border:`1px solid ${E.border}`,
              ...
              <StepComp data={data} set={set} onSubmit={handleSubmit}/>
              {step < 6 && (
...
export default function App() {
  ...
  return auth
    ? <DashboardLayout onLogout={() => setAuth(false)} />
